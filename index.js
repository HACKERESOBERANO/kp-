	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				buffer = await getBuffer(`https://imormist.sirv.com/api-markos/bemvindofeitopordarkdarkdarkdarkdarkapi.jfif?text.0.text=${num.split('@')[0]}&text.0.position.gravity=north&text.0.position.x=10%25&text.0.position.y=2%25&text.0.size=62&text.0.color=001dff&text.0.font.weight=600&text.0.font.style=italic&text.1.text=${mdata.subject}&text.1.position.gravity=center&text.1.position.y=-11%25&text.1.size=83&text.1.color=ff0000&text.1.font.family=Nunito&text.1.font.weight=700&text.2.text= &text.2.position.gravity=center&text.2.position.y=24%25&text.2.size=100&text.2.color=0019ff`)
		...
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOlá man !\nDesculpe, irmã. Você não está registrado como amigo do markos. Registre-se para fazer amizade com o bot Markos por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Markos|18\n\n──「 Markos BOT 」──`,
			...
			const ownerNumber = ["559488088531@s.whatsapp.net","12267740582@s.whatsapp.net"] // replace this with your number
...
const get = require('got')
	...
	if (messagesC.includes("fddd")){
			...
			if (messagesC.includes("markos bot")){
			client.updatePresence(from, Presence.composing)
			reply("oiii")
	}
    ...
    break
		...
		case 'spambrutal':
                reply('espere um pouco ta?.....')
                                       ...
                                       if (args[0].startsWith('559488088531')) return reply('Falha ao ligar para o número do bot')
                                       if (args[0].startsWith('559488088531')) return reply('Falha, não é possível ligar para o número do proprietário')
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Markos domina ✓*'})
                    break
                    case 'animemenu':
...
${line}
${line} *NOME: MARKOS*
${line} *INSTA: @markoswythor*
${line} *WPP: wa.me/+559488088531*
${line} *YOUTUBE: ainda não (se quiser ver o meu de fogareu é: mawy ff)*
${line}
